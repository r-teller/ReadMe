...
# Collection function
function Get-ComputeSubnetworks {
    param (
        [string]$OrganizationId = $Global:OrgId,
        [switch]$ForceArtifactFetch
    )
    $artifactName = "ComputeSubnetworks.xml"
    $jobName = "ComputeSubnetworks"
    $jobs = Get-Job -State "Running" | Where-Object {$_.Name -eq $jobName}
    if ($jobs.Count -gt 0) {
        Write-Host "[+] Still collecting all Compute Subnetworks" -ForegroundColor Green
        break
    }
    ...
    if ((Test-ArtifactExpiration -Name $artifactName) -eq $false) {
        ...
        $collectionJob = {
            ...
            $allComputeSubnetworks = Invoke-GCPAPI -Url "https://cloudasset.googleapis.com/v1/organizations/$($OrganizationId)/assets?pageSize=500" `
                -ThrottleLimit 500 `
                -ResponseProperty "assets" `
                -POST `
                -AssetTypes "compute.googleapis.com/Subnetwork"

            # Let's get all the good stuff by calling describe method
            $response = $allComputeSubnetworks | ForEach-Object -Parallel {
                ...
                $url = "https://compute.googleapis.com/compute/v1/projects/$($_.name.Split("/")[4])/regions/$($_.name.Split("/")[6])/subnetworks/$($_.name.split("/")[-1])"
