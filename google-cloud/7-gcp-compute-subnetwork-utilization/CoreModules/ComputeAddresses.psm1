...
# Collection function
function Get-ComputeAddresses {
    param (
        [string]$OrganizationId = $Global:OrgId,
        [switch]$ForceArtifactFetch
    )
    $artifactName = "ComputeAddresses.xml"
    $jobName = "ComputeAddresses"
    $jobs = Get-Job -State "Running" | Where-Object {$_.Name -eq $jobName}
    if ($jobs.Count -gt 0) {
        Write-Host "[+] Still collecting all Compute Addresses" -ForegroundColor Green
        break
    }
    ...
    if ((Test-ArtifactExpiration -Name $artifactName) -eq $false) {
        ...
        $collectionJob = {
            ...
            $allComputeAddresses = Invoke-GCPAPI -Url "https://cloudasset.googleapis.com/v1/organizations/$($OrganizationId)/assets?pageSize=500" `
                -ThrottleLimit 500 `
            ...
            $response = $allComputeAddresses | ForEach-Object -Parallel {
                ...
                $_name = $_.name.Split("/")
                $url = "https://compute.googleapis.com/compute/v1/projects/$($_name[4])/regions/$($_name[6])/addresses/$($_name[-1])"
