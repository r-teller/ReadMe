...
# Collection function
function Get-ComputeForwardingRules {
    param (
        [string]$OrganizationId = $Global:OrgId,
        [switch]$ForceArtifactFetch
    )
    $artifactName = "ComputeForwardingRules.xml"
    $jobName = "ComputeForwardingRules"
    $jobs = Get-Job -State "Running" | Where-Object {$_.Name -eq $jobName}
    if ($jobs.Count -gt 0) {
        Write-Host "[+] Still collecting all Compute ForwardingRules" -ForegroundColor Green
        break
    }
    ...
    if ((Test-ArtifactExpiration -Name $artifactName) -eq $false) {
        ...
        $collectionJob = {
            ...
            $allComputeForwardingRules = Invoke-GCPAPI -Url "https://cloudasset.googleapis.com/v1/organizations/$($OrganizationId)/assets?pageSize=500" `
                -ThrottleLimit 500 `
                -ResponseProperty "assets" `
                -POST `
                -AssetTypes "compute.googleapis.com/ForwardingRule"

            # Let's get all the good stuff by calling describe method
            $response = $allComputeForwardingRules | ForEach-Object -Parallel {
                ...
                $_name = $_.name.Split("/")
                $url = "https://compute.googleapis.com/compute/v1/projects/$($_name[4])/regions/$($_name[6])/forwardingRules/$($_name[-1])"
