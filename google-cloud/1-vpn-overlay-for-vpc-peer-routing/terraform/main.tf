    ...
    vpc_peer_mappings = zipmap(keys(var.environments),[
        for environment in keys(var.environments):[
            for peer in var.environments[environment].vpc_peers: {
                ...
                local=module.environment_standup[environment].network_self_link
                remote=module.environment_standup[peer].network_self_link
            }
        ]
    ])
    vpn_peerings = flatten([
        for vpn_peers in var.vpn_peerings: {          
            first = {
                ...
                network = module.environment_standup[vpn_peers.first].network_self_link,
            ...
            second = {
                ...
                network = module.environment_standup[vpn_peers.second].network_self_link,
...
module "jump_host" {
    ...
    environment = each.key
    vpc_name = module.environment_standup[each.key].network_self_link
    project_id = module.environment_standup[each.key].project_id
    random_id = module.environment_standup[each.key].id
    subnetwork = module.environment_standup[each.key].subnetwork_self_link
